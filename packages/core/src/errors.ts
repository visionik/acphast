 ...
 */
export function isTransientError(error: ACPError | Error): boolean {
  if ('code' in error) {
    return (
      error.code === ACPErrorCode.RateLimited ||
 ...
 */
export function isPermanentError(error: ACPError | Error): boolean {
  if ('code' in error) {
    return (
      error.code === ACPErrorCode.AuthFailed ||
...
export class BackendError extends AcphastError {
  constructor(backend: string, originalError: unknown) {
    super(
      `Backend "${backend}" returned an error`,
      ACPErrorCode.BackendError,
      { backend, originalError }
    );
...
export class CapabilityUnsupportedError extends AcphastError {
  constructor(capability: string, backend: string) {
    super(
      `Capability "${capability}" not supported by backend "${backend}"`,
      ACPErrorCode.CapabilityUnsupported,
      { capability, backend }
    );
...
export class ContextExceededError extends AcphastError {
  constructor(backend: string, tokenCount: number, maxTokens: number) {
    super(
      `Context window exceeded for backend "${backend}"`,
      ACPErrorCode.ContextExceeded,
      { backend, tokenCount, maxTokens }
    );
...
export class AuthenticationError extends AcphastError {
  constructor(backend: string) {
    super(
      `Authentication failed for backend "${backend}"`,
      ACPErrorCode.AuthFailed,
      { backend }
    );
